...
export function CompletedBillModal({ isOpen, onClose, sale, onNewBill }) {
  ...
  return (
    <>
      {/* 1. SCREEN MODAL DIALOG - COMPLETELY HIDDEN ON PRINT */}
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 max-h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto bg-neutral-950 border border-neutral-800 rounded-xl p-4 font-mono text-xs text-neutral-300 space-y-3">
            <div className="text-center space-y-1 border-b border-dashed border-neutral-800 pb-3">
              ...
              <p className="text-[10px] text-neutral-500 pt-0.5 font-bold">TAX INVOICE / RETAIL RECEIPT</p>
              {sale.createdAt && (
          ...
          <div className="space-y-2 pt-1 print:hidden">
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={handlePrint}
                className="py-2 px-2.5 text-xs font-medium text-neutral-200 bg-neutral-800 hover:bg-neutral-700 rounded-xl transition-colors flex items-center justify-center gap-1.5 border border-neutral-700 cursor-pointer"
              >
              ...
              <button
                ...
                className="py-2 px-2.5 text-xs font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
            ...
            <button
              type="button"
              onClick={onNewBill}
              className="w-full py-2.5 px-3 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-md shadow-emerald-600/20 cursor-pointer"
            >
      ...
      {/* 2. DEDICATED PRINT RECEIPT - ONLY VISIBLE DURING PRINT (PROFESSIONAL 80mm THERMAL RECEIPT) */}
      <div
        id="pos-thermal-receipt"
        className="hidden print:block print:w-[78mm] print:max-w-[78mm] print:mx-auto print:my-0 print:p-0 print:bg-white print:text-black print:overflow-visible"
      >
        <div className="bg-white text-black font-mono text-[11px] leading-tight p-2 space-y-2">
          {/* Store Header */}
          <div className="text-center space-y-0.5 border-b border-dashed border-black pb-2">
            <h1 className="text-sm font-bold uppercase tracking-wider font-sans text-black">
              {storeName}
            </h1>
            {storeAddress && <p className="text-[10px] text-neutral-800">{storeAddress}</p>}
            {storePhone && <p className="text-[10px] text-neutral-800">Tel: {storePhone}</p>}
            {store.taxId && <p className="text-[10px] text-neutral-800">GSTIN: {store.taxId}</p>}
            <p className="text-[10px] font-bold tracking-wide mt-1 text-black">TAX INVOICE / RETAIL RECEIPT</p>
          ...
          <div className="space-y-0.5 text-[10px] border-b border-dashed border-black pb-1.5">
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-black font-medium">{sale.customer?.name || 'Walk-in'}</span>
            </div>
            {sale.customer?.phone && (
              <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <span className="text-neutral-700">Cashier:</span>
              <span className="text-black">{sale.billedBy?.name || 'Cashier'}</span>
            </div>
          </div>

          {/* Itemized Table */}
          <div className="border-b border-dashed border-black pb-2">
            <div className="flex justify-between text-[10px] font-bold uppercase border-b border-black pb-1 mb-1">
              <span className="w-[45%] text-left">Item</span>
              <span className="w-[30%] text-right">Qty x Rate</span>
            ...
            {sale.items?.map((item, idx) => (
              <div key={idx} className="py-0.5 text-[10px]">
                <div className="flex justify-between items-start">
                  <div className="w-[45%]">
                    <span className="font-medium text-black block break-words">{item.name}</span>
                    {item.packSize && <span className="text-[9px] text-neutral-700">{item.packSize}</span>}
                  </div>
                  <div className="w-[30%] text-right text-neutral-800 whitespace-nowrap">
                    {item.quantity} x {currency}{item.unitPrice}
                  </div>
                  <div className="w-[25%] text-right font-bold text-black whitespace-nowrap">
                    {currency}{item.lineTotal}
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-0.5 text-[11px] border-b border-dashed border-black pb-2">
            ...
            <div className="flex justify-between text-xs font-bold pt-1 border-t border-black text-black">
              <span>GRAND TOTAL:</span>
              <span className="text-sm font-black">{currency}{sale.grandTotal}</span>
            </div>
          </div>

          {/* Payment Method */}
          <div className="text-[10px] space-y-0.5 border-b border-dashed border-black pb-1.5">
            <div className="flex justify-between">
          ...
          {/* Footer Message */}
          <div className="text-center pt-1 text-[9px] text-neutral-800 space-y-0.5">
            <p className="font-medium">{footerMessage}</p>
            <p className="text-[8px] text-neutral-600">Powered by RetailFlow</p>
          </div>
        </div>
      </div>
    </>
